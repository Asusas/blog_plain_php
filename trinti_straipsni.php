...
if (isset($_POST['tvirtinti'])) {
    ...
    $PostId_URL = $_GET["Trinti"];
    $Query = "DELETE FROM admin_panel WHERE id='$PostId_URL '";
    $Execute = mysqli_query($Conn, $Query);
    move_uploaded_file($_FILES["Image"]["tmp_name"], $Img_upload);
    if ($Execute) {
        $_SESSION["SucessMessage"] = "Straipsnis istrintas sekmingai !";
        redirect_to("meniu.php");
    } else {
        $_SESSION["ErrorMessage"] = "Straipsnio istrinti nepavyko !";
        redirect_to("meniu.php");
    }
...
?>
                    ...
                    <form action="trinti_straipsni.php?Trinti=<?php echo $PostId_URL ?>" method="post"
                        enctype="multipart/form-data">
                        <fieldset>
                            <div class="form-group">
                                <label for="pavadinimas">Pavadinimas: </label>
                                <input disabled value="<?php echo $TitleEdit ?>" class="form-control" type="text"
                            ...
                            <div class="form-group">
                                ...
                                <select disabled class="form-control" id="kategorijos_parinkimas" name="kategorija">
                                    <?php
global $Conn;
...
while ($Data = mysqli_fetch_array($result)) {
    ...
    ?>
                            ...
                            <div class="form-group">
                                ...
                                <br>
                                <img disabled style="width: 300px; height: 150px"
                                    src="img_upload/<?php echo $ImgEdit ?>" alt="">
                                <br>
                                <label for="paveiksliuko_parinkimas">Pasirinkite paveiksleli: </label>
                                <input disabled type="File" class="form-control" name="Image"
                                    id="paveiksliuko_parinkimas">
                            </div>

                            <div class="form-group">

                                <label for="straipsnio_laukas">Straipsnis: </label>
                                <textarea disabled class="form-control" name="siusti" id="straipsnio_laukas">
                                <?php echo $PostEdit ?></textarea>
                            </div>

                            <input class="btn btn-danger btn-block" type="Submit" name="tvirtinti"
                                value="Trinti straipsni">
                        </fieldset>
                        <br>
                    </form>
                </div>
